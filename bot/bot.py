...
"""  
    with open(sitemap_file, "w", encoding="utf-8") as f:  
        f.write(sitemap)  
    print("sitemap.xml обновлён")  def generate_rss(fresh_news):
rss_items = ""
...
"""  
    with open("public/rss.xml", "w", encoding="utf-8") as f:  
        f.write(rss)  
    print("rss.xml обновлён")  def load_seen_ids():
...
"""  
                ...
                archived_count += 1  
                print(f"АРХИВ: {full_text[:30]}... ({date_str})")  media_paths = re.findall(r"src=['\"](.*?)['\"]", block)  
...
</html>"""  with open("public/archive.html", "w", encoding="utf-8") as f:  
